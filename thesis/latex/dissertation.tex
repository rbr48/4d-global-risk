...
To guarantee computational tractability and empirical rigor across $\sim$3,650 daily rolling origins, the methodology introduces a \textbf{Two-Tier Inference Protocol}: daily rolling estimation uses analytical Dynamic Linear Models (Kalman Filter and Fixed-Interval Smoother), while quarterly checkpoint origins undergo full Bayesian MCMC with hierarchical shrinkage priors. Latent factor states are identified via strict sign constraints ($\lambda_{\text{stress}} > 0$) to eliminate rolling polarity inversions.
