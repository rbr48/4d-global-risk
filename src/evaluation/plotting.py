...
def plot_dgrs_trajectory(
    df_dgrs: pd.DataFrame, output_path: str = "results/figures/figure_4_dgrs_trajectory.png", dpi: int = 300
):
    """
    ...
    ax.axhline(
        q85, color="#d62728", linestyle="--", linewidth=1.5, label=f"85th Percentile Stress Threshold ({q85:.2f})"
    )

    ax.set_title(
        "Figure 4: Dynamic Global Risk State (DGRS) Latent Factor Trajectory", fontsize=13, fontweight="bold", pad=12
    )
...
def plot_propagation_heatmaps(
    ...
    axes[0].imshow(df_pi_1.values, cmap=cmap, vmin=0.0, vmax=1.0)
    ...
    fig.suptitle(
        "Figure 7: Cross-Domain Dynamic Propagation Matrices (Pi_1 vs Pi_7)", fontsize=14, fontweight="bold", y=1.02
    )
...
def plot_model_comparison_bars(
    perf_table: pd.DataFrame, output_path: str = "results/figures/figure_11_model_comparison.png", dpi: int = 300
):
    """
    ...
    fig.suptitle(
        "Figure 11: Out-of-Sample Model Ladder Verification (Horizon = 1 Day)", fontsize=13, fontweight="bold", y=0.98
    )
    plt.tight_layout()
    plt.savefig(output_path)
    plt.close(fig)
